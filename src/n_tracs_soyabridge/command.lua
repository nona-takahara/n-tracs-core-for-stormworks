...
COMMANDS["aspect"] = {
    ...
    command = (function(args, is_admin, is_auth, peer_id)
        local nm = args[2]
        if sys.nt.levers[nm] then
            table.insert(DELAY_ANNOUNE, function()
                Announce("Singal \"" .. nm .. "\" aspect: " .. tostring(sys.nt.levers[nm].aspect), peer_id)
            end)
        else
...
COMMANDS["set"] = {
    ...
    command = (function(args, is_admin, is_auth, peer_id)
        local nm = args[2]
        if sys.nt.levers[nm] then
            if sys.nt.levers[nm].name == "Lever" then
                ---@diagnostic disable-next-line: param-type-mismatch
                sys.nt.levers[nm]:setInput(true, not (nm == "WAK1R" or nm == "WAK4L" or nm == "SGN1R" or nm == "SGN4L"))
...
COMMANDS["reset"] = {
    ...
    command = (function(args, is_admin, is_auth, peer_id)
        ...
        if sys.nt.levers[nm] then
            if sys.nt.levers[nm].name == "Lever" then
                ---@diagnostic disable-next-line: param-type-mismatch
                sys.nt.levers[nm]:setInput(false, false)
...
COMMANDS["debug"] = {
    ...
    command = (function(args, is_admin, is_auth, peer_id)
        ...
        if sys.nt.switches[nm] then
            table.insert(DELAY_ANNOUNE, function()
                Announce("Switch \"" .. nm .. "\" route: " .. tostring(sys.nt.switches[nm]:getRealRoute()), peer_id)
            end)
        end
    end)
}
